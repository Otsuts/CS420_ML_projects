...
def write_log(w, args):
    file_name = args.log + '/'+args.model + '/' + datetime.date.today().strftime('%m%d') + \
        f"_LR{args.lr}_WD{args.weight_decay}_EXP{args.exp_name}.log"
    if not os.path.exists(args.log + '/' + args.model + '/'):
        os.mkdir(args.log + '/' + args.model + '/')
